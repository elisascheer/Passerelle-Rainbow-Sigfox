...
function check_temperature(data,device){
    ...
    req.on("end",function(result){
        ...
        //var bubblejid=result.rows[0]
        //var bubble=rainbowSDK.bubbles.getBubbleByJid()
        //var name_patient=client.query("SELECT * FROM sensors WHERE device='"+device+"'");
        /*for(i=0;i<result.rows.length;i++){
            messageSent = rainbowSDK.im.sendMessageToJid("Attention, la température du patient a atteint "+data+"°C ", result.rows[i].jid);
...
function warning(message,value){
    ...
    /*ar search_name=client.query("SELECT * FROM users WHERE name='"+name+"'");
    search_name.on("row",function(row,result){
        result.addRow(row);
    });
    search_name.on("end",function(result){
...
rainbowSDK.events.on('rainbow_onmessagereceived', function(message) {
    ...
    if(message.type == "chat") {
        ...
        else if(chaine.indexOf("link_patient")==0){ //si on entre la commande link_patient
            //link ID name
        ...
        else if(chaine.indexOf("link_device")==0){ //si on entre la commande link_device
            //link ID name
            var split=chaine.split(" ");
